...
public class PredationsModMenuIntegration implements ModMenuApi {
    ...
    public ConfigScreenFactory<?> getModConfigScreenFactory() {
        return parent -> {
            ...
            // Kicking Llama
            ExtraConfig.KickingLlamaConfig llama = extraConfig.kickingLlama;
            SubCategoryBuilder llamaSub = entryBuilder.startSubCategory(Text.of("Kicking Llama"));
            llamaSub.add(entryBuilder.startBooleanToggle(Text.of("Enabled"), llama.enabled).setDefaultValue(true).setSaveConsumer(v -> llama.enabled = v).build());

            // Updated Targeting Toggles
            llamaSub.add(entryBuilder.startBooleanToggle(Text.of("Kick Undead Near"), llama.llamaKickUndeadWhenNear).setDefaultValue(true).setSaveConsumer(v -> llama.llamaKickUndeadWhenNear = v).build());
            llamaSub.add(entryBuilder.startBooleanToggle(Text.of("Kick Arthropods Near"), llama.llamaKickArthropodWhenNear).setDefaultValue(true).setSaveConsumer(v -> llama.llamaKickArthropodWhenNear = v).build());
            llamaSub.add(entryBuilder.startBooleanToggle(Text.of("Kick If Attacked"), llama.llamaKickIfAttacked).setDefaultValue(true).setSaveConsumer(v -> llama.llamaKickIfAttacked = v).build());


            // New Damage Settings
            llamaSub.add(entryBuilder.startFloatField(Text.of("Dmg Easy"), llama.llamaKickDamageHeartsEasy).setDefaultValue(0.5f).setSaveConsumer(v -> llama.llamaKickDamageHeartsEasy = v).build());
            llamaSub.add(entryBuilder.startFloatField(Text.of("Dmg Normal"), llama.llamaKickDamageHeartsNormal).setDefaultValue(1.0f).setSaveConsumer(v -> llama.llamaKickDamageHeartsNormal = v).build());
            llamaSub.add(entryBuilder.startFloatField(Text.of("Dmg Hard"), llama.llamaKickDamageHeartsHard).setDefaultValue(1.5f).setSaveConsumer(v -> llama.llamaKickDamageHeartsHard = v).build());

            // Physics
            llamaSub.add(entryBuilder.startDoubleField(Text.of("Kick Horiz Vel"), llama.llamaKickHorizontalVelocity).setDefaultValue(0.28).setSaveConsumer(v -> llama.llamaKickHorizontalVelocity = v).build());
            llamaSub.add(entryBuilder.startDoubleField(Text.of("Kick Vert Vel"), llama.llamaKickVerticalVelocity).setDefaultValue(1.2).setSaveConsumer(v -> llama.llamaKickVerticalVelocity = v).build());


            // Extras
            llamaSub.add(entryBuilder.startStrList(Text.of("Extra Targets"), llama.llamaExtraEntities).setSaveConsumer(l -> { llama.llamaExtraEntities.clear(); llama.llamaExtraEntities.addAll(l); }).build());
            mechCat.addEntry(llamaSub.build());
            // Milk Change
            ...
            rabiesSub.add(entryBuilder.startIntField(Text.of("Hydrophobia Start Tick"), rabies.rabiesHydrophobiaStartTime).setDefaultValue(60000).setSaveConsumer(v -> rabies.rabiesHydrophobiaStartTime = v).build());
