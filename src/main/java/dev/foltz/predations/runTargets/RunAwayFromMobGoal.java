...
public class RunAwayFromMobGoal extends Goal {
    ...
    private final DetectMobTypesGoal detectorGoal;
    ...
    public RunAwayFromMobGoal(PathAwareEntity mob, DetectMobTypesGoal detectorGoal) {
        this.mob = mob;
    ...
    public boolean canStart() {
        ...
        return this.detectorGoal.shouldFlee() && this.detectorGoal.getTarget() != null;
    }
    ...
    public void tick() {
        ...
        if (this.mob.isLeashed() && ConfigManager.leashingStopTheRunning(this.mob)) {
            try {
                double runSpeed = ConfigManager.runSpeed(this.mob);
                this.nav.setSpeed((float)(runSpeed - (runSpeed - 1.0)));
                this.nav.stop();
        ...
        else if (this.detectorGoal.shouldFlee()) {
